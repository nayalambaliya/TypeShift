...
enum GroqError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .parseFailure:     return "Unexpected response format"
        }
    }
}

func callGroq(text: String, instruction: String, apiKey: String) async throws -> String {
    guard !apiKey.isEmpty else { throw GroqError.noKey }
    ...
    if let http = response as? HTTPURLResponse, http.statusCode != 200 {
        throw GroqError.badStatus(http.statusCode)
    }
    ...
    else { throw GroqError.parseFailure }
